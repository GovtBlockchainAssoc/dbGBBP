...
INSERT INTO @Txs (TxId, My_Chain, My_Addr, Token, Amount, Memo, Their_Chain, Their_Addr, Addr, Date_Time, Updated) 
	(SELECT tx.Id, FromChain, FromAddr, Token, -1 * Value, Memo, DestChain, DestAddr, DestAddr, FORMAT(Updated, 'M/d/yy h:mm:ss tt'), Updated 
	FROM TokenTxs tx INNER JOIN MemberAddrs a ON tx.FromAddr = a.Address WHERE MemberId = @pMemberId
	UNION
	SELECT tx.Id, DestChain, DestAddr, Token, Value, Memo, FromChain, FromAddr, FromAddr, FORMAT(Updated, 'M/d/yy h:mm:ss tt'), Updated 
	FROM TokenTxs tx INNER JOIN MemberAddrs a ON tx.ToAddr = a.Address WHERE MemberId = @pMemberId)
	ORDER BY Updated

UPDATE @Txs SET My_Chain = 'GBA Hub' WHERE My_Chain = 'Core PoA'
UPDATE @Txs SET Their_Chain = 'GBA Hub' WHERE Their_Chain = 'Core PoA'
